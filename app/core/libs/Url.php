...
{    
    ...
    static function redirect(string $url, int $http_code = 307){
        Response::redirect($url, $http_code);
    } 
    ...
    */
    ...
    static function currentUrl(){
        // Si estamos en un entorno de línea de comandos (CLI) o cron,
        // no podemos construir la URL de la misma manera.
        if (is_cli() || !isset($_SERVER['HTTP_HOST'])) {
            // Devolvemos la URL base del sitio, que es la opción más segura.
            return get_site_url();
        }

        // Si es una visita web normal, construimos la URL completa.
    ...
    {
        // Si no se proporciona una URL, usamos la URL del sitio de WordPress como fuente fiable.
        if (is_null($url)) {
            $url = get_site_url(); 
        }

        // Si la URL está vacía, no podemos continuar.
        if (empty($url)){
            return '';
        }
        ...
        // Nos aseguramos de que 'host' exista antes de usarlo.
        ...
        if ($include_protocol) {
            // Aseguramos que 'scheme' exista antes de usarlo.
